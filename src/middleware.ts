...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  response.headers.set(
    ...
    `default-src 'self'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'; object-src 'none'; script-src 'self' 'nonce-${nonce}' https://challenges.cloudflare.com https://static.cloudflareinsights.com https://www.googletagmanager.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; connect-src 'self' https://challenges.cloudflare.com https://cloudflareinsights.com https://www.google-analytics.com https://www.googletagmanager.com; frame-src 'self' http: https: data: blob: https://challenges.cloudflare.com; require-trusted-types-for 'script'; trusted-types 'allow-duplicates' default; report-uri /api/csp-report; upgrade-insecure-requests`
  );
