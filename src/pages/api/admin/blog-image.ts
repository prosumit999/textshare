...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    recordAudit({ actor: admin.email, action: 'upload', resourceType: 'blog_image', resourceId: stored.id, details: `${image.bytes} bytes; MongoDB Base64` });
    return Response.json({ url: stored.url }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
