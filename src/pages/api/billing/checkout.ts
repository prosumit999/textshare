...
export const POST: APIRoute = async ({ cookies, request, url }) => {
  ...
  if (plan !== "monthly" && plan !== "annual")
    return Response.json({ error: "Choose a valid billing plan." }, { status: 400 });
  try {
    ...
    return Response.json({ error: "Checkout could not be started. Try again shortly." }, { status: 503 });
  }
};
