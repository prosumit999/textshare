...
export type CleanupReport = {
  ...
  oversizedShares: number;
  expiredGuestShares: number;
  oversizedGuestShares: number;
  orphanedPayloads: number;
  orphanedBlogImages: number;
  failures: number;
};
...
export async function runCleanupWorker(): Promise<CleanupReport> {
  ...
  const report: CleanupReport = {
    ...
    expiredShares: 0,
    oversizedShares: 0,
    expiredGuestShares: 0,
    oversizedGuestShares: 0,
    orphanedPayloads: 0,
    orphanedBlogImages: 0,
    failures: 0,
  };
  const locks = db.collection("workerLocks");
  const lockUntil = new Date(startedAt.getTime() + LOCK_TTL_MS);
  let acquired = await locks.findOneAndUpdate(
    { _id: LOCK_ID as any, expiresAt: { $lte: startedAt } },
    { $set: { owner: runId, acquiredAt: startedAt, expiresAt: lockUntil } },
  ...
  try {
    const memoryStore = globalThis.textSharesStore as
      Map<string, any> | undefined;
    for (const [slug, share] of memoryStore || []) {
      const expired =
        new Date(share.expiryDate).getTime() <= startedAt.getTime();
      ...
      if (expired) report.expiredGuestShares += 1;
      else report.oversizedGuestShares += 1;
      await durableAudit(
        expired ? "expire_guest_share" : "auto_kill_oversized_guest",
        "share",
        slug,
        `${share.contentType}; ${bytes} bytes`,
      );
    }
    ...
    const candidates = await db
      ...
      .find({
        $or: [
          { expiryDate: { $lte: startedAt } },
          { sizeBytes: { $gt: MAX_SHARE_BYTES } },
        ],
      })
      .sort({ expiryDate: 1 })
    ...
    for (const share of candidates) {
      ...
      try {
        ...
        await durableAudit(
          expired ? "expire_share" : "auto_kill_oversized",
          "share",
          String(share.slug),
          `${share.contentType}; ${Number(share.sizeBytes || 0)} bytes; GridFS payload removed`,
        );
      } catch (error) {
        report.failures += 1;
        await durableAudit(
    ...
    const oldFiles = await db
      .collection("sharePayloads.files")
      .find({ uploadDate: { $lte: orphanCutoff } })
      .limit(MAX_RECORDS_PER_RUN)
      .toArray();
    for (const file of oldFiles) {
      if (
        await db
          .collection("shares")
          .findOne({ contentFileId: file._id }, { projection: { _id: 1 } })
      )
        continue;
      try {
        ...
        await durableAudit(
          "delete_orphaned_payload",
          "gridfs_payload",
          String(file._id),
          String(file.filename || "encrypted share payload"),
        );
      } catch (error) {
        report.failures += 1;
        await durableAudit(
    ...
    const referencedBlogImages = new Set(
      (await getBlogPosts())
        .map(
          (post) =>
            post.coverImage.match(/^\/api\/blog-images\/([a-f0-9]{24})$/)?.[1],
        )
        .filter(Boolean),
    );
    ...
    const blogImages = await db
      ...
      .find(
        { createdAt: { $lte: blogCutoff } },
        { projection: { _id: 1, bytes: 1 } },
      )
      .limit(MAX_RECORDS_PER_RUN)
      .toArray();
    for (const image of blogImages) {
      ...
      await durableAudit(
        "delete_orphaned_blog_image",
        "blog_image",
        String(image._id),
        `${Number(image.bytes || 0)} bytes; older than grace period`,
      );
    }
    ...
    await locks
      .deleteOne({ _id: LOCK_ID as any, owner: runId })
      .catch(() => undefined);
  }
}
