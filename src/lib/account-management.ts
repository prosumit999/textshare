...
export async function changeAccountName(user: User, name: string) {
  ...
  return (await db.collection("users").updateOne({ email: user.email, isAdmin: false }, { $set: { name: clean } })).modifiedCount > 0;
}

export async function changeAccountPassword(user: User, currentPassword: string, newPassword: string) {
  if (user.isAdmin || !(await verifyUserPassword(user, currentPassword))) return false;
  ...
  await sendSecurityAlert("Your password was changed", "Your TextShare password was changed from account settings and all devices were signed out.", user.email).catch(() => false);
  return true;
}
...
export async function deleteAccount(user: User, password: string) {
  ...
  if (user.billing?.subscriptionId && !["canceled", "incomplete_expired"].includes(user.billing.status || "")) {
  ...
  await Promise.all([
    ...
    db.collection("emailChangeTokens").deleteMany({ $or: [{ oldEmail: user.email }, { newEmail: user.email }] }),
  ]);
  return (await db.collection("users").deleteOne({ email: user.email, isAdmin: false })).deletedCount > 0;
}
...
export async function exportAccountData(user: User) {
  ...
  const [shares, sessions] = await Promise.all([
    ...
    db.collection("sessions").find({ email: user.email }, { projection: { _id: 0, tokenHash: 0 } }).toArray(),
  ]);
  return {
