  ...
  if (!redisPromise) {
    redisPromise = (async () => {
      ...
      client.on('error', (error) => {
        console.error(JSON.stringify({ event: 'redis_error', at: new Date().toISOString(), message: error.message }));
      });
      await client.connect();
      return client as RedisClientType;
    })().catch((error) => {
      redisPromise = null;
      console.error(JSON.stringify({ event: 'redis_connection_failed', at: new Date().toISOString(), message: error.message }));
      return null;
    });
  }
...
export function getClientIp(request: Request): string {
  const trustProxy = (import.meta.env.TRUST_PROXY || process.env.TRUST_PROXY) === 'true';
  if (trustProxy) {
...
export async function checkRateLimit(
  ...
  if (import.meta.env.PROD) {
    console.error(JSON.stringify({ event: 'rate_limit_store_unavailable', scope, at: new Date().toISOString() }));
...
export function rateLimitResponse(retryAfter: number, message = 'Too many requests. Please try again later.') {
...
export function logSecurityEvent(event: string, request: Request, details: Record<string, unknown> = {}) {
  ...
  recordSecuritySignal(event, ip, details);
  console.warn(JSON.stringify({ event, at: new Date().toISOString(), ip, ...details }));
}
