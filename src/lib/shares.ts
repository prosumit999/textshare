...
function measuredShareBytes(share: any) {
  ...
  const textBytes = typeof share?.textContent === 'string' ? Buffer.byteLength(share.textContent, 'utf8') : 0;
  let imageBytes = 0;
  if (typeof share?.imageSrc === 'string' && share.imageSrc.startsWith('data:')) {
    ...
    try {
      for (const share of await listPersistentShares()) {
        ...
        if (await deletePersistentShare(share.slug)) {
          removed += 1;
          recordAudit({ actor: 'system', action: 'auto_kill_oversized', resourceType: 'share', resourceId: share.slug, details: `${share.contentType || 'unknown'}; ${bytes} bytes; limit ${MAX_SHARE_BYTES} bytes` });
        }
      }
    } catch (error) {
