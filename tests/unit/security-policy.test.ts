...
describe("share and content policy", () => {
  ...
  it("calculates every supported expiry deterministically", () => {
    ...
    expect(calculateExpiry("1h", now).toISOString()).toBe(
      "2026-01-01T01:00:00.000Z",
    );
    expect(calculateExpiry("3h", now).toISOString()).toBe(
      "2026-01-01T03:00:00.000Z",
    );
    expect(calculateExpiry("24h", now).toISOString()).toBe(
      "2026-01-02T00:00:00.000Z",
    );
    expect(calculateExpiry("1w", now).toISOString()).toBe(
      "2026-01-08T00:00:00.000Z",
    );
    expect(calculateExpiry("burn", now).toISOString()).toBe(
      "2026-01-01T00:05:00.000Z",
    );
  });
  it("enforces the 50 MB share ceiling", () =>
  ...
  it("removes executable blog markup and unsafe URLs", () => {
    const clean = sanitizeBlogHtml(
      '<script>alert(1)</script><p onclick="x()">Safe</p><img src="javascript:alert(1)"><a href="https://example.com">Link</a>',
    );
    expect(clean).not.toContain("script");
    expect(clean).not.toContain("onclick");
    expect(clean).not.toContain("javascript:");
    expect(clean).toContain('rel="noopener noreferrer"');
    expect(blogTextExcerpt("<p>Hello <strong>world</strong></p>")).toBe(
      "Hello world",
    );
  });
});
