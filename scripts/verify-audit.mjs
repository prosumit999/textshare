...
const keys = (process.env.AUDIT_LOG_KEYS || process.env.AUDIT_LOG_SECRET || "")
  .split(",")
  .map((v) => v.trim())
  .filter(Boolean);
...
for await (const row of db
  ...
  const key = keyring.get(row.keyId);
  if (!key) throw new Error(`Missing audit key ${row.keyId}.`);
  ...
  if (actual.length !== expected.length || !timingSafeEqual(actual, expected))
    throw new Error(`Invalid signature at sequence ${row.sequence}.`);
...
await client.close();
console.log(
  `Verified ${checked} audit records${outputPath ? ` and exported to ${outputPath}` : ""}.`,
);
